...
@Component({
  selector: 'app-declined-appointment-refunds',
  templateUrl: './declined-appointment-refunds.component.html',
  styleUrls: ['./declined-appointment-refunds.component.scss']
})
export class DeclinedAppointmentRefundsComponent implements AfterViewInit, AfterContentChecked {
  declinedRefunds:any=[];
...
constructor(private refundsService:RefundsService, private dialog:MatDialog, private spinner:SpinnerService,private cdr:ChangeDetectorRef, private snackbar:MatSnackBar){
  ...
  ngAfterViewInit() {
    ...
    this.refundsService.getUserRefunds().subscribe({
     ...
     next:(res:any)=>{
       this.declinedRefunds=res;
     if(this.declinedRefunds.length==0){
       this.openSnackBar('There are no declined appointment refunds to show!','OK');
     }
     else{
     this.dataSource = new MatTableDataSource(this.declinedRefunds);
  ...
  error:(e)=>{
   this.spinner.hide();
   console.log(e);
   this.openSnackBar('Error getting declined appointment refunds! Please try again.','OK');
  }
    });
  }
