...
@Component({
  selector: 'app-requested-appointment-refunds',
  templateUrl: './requested-appointment-refunds.component.html',
  styleUrls: ['./requested-appointment-refunds.component.scss']
})

export class RequestedAppointmentRefundsComponent implements AfterContentChecked, AfterViewInit {
  requestedRefunds:any=[];
  ...
  ngAfterViewInit(){
    ...
    this.refundsService.getUserRefunds().subscribe({
     ...
     next:(res:any)=>{
       this.requestedRefunds=res;
     if(this.requestedRefunds.length==0){
       this.openSnackBar('There are no requested appointment refunds to show!','OK');
     }
     else{
     this.dataSource = new MatTableDataSource(this.requestedRefunds);
